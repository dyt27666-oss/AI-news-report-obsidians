...
多 agent 编程框架，适合观察 agent orchestration 和 eval loop。 今日作为 watched repo fallback 被纳入；由于 GitHub Search 403，它不是完整全网排名，但元数据来自 direct repo GET。

## TL;DR

- repo：`microsoft/autogen`
- stars / forks：59851 / 9009
- language：Python
- updated_at：2026-07-20T22:43:02Z
- stars_delta：30，口径为 direct watched repo fallback vs historical snapshot，非完整全网日增。
...
| 原文链接 | [https://github.com/microsoft/autogen](https://github.com/microsoft/autogen) |
...
flowchart TB
  ...
  subgraph Project[项目观察]
    P1[microsoft/autogen]
    P2[Stars 59851]
    P3[Language Python]
    P4[Updated 2026-07-20]
  end
...
| 维度 | 判断 | 原因 |
|---|---|---|
| AI Infra 相关性 | 中高 | 多 agent 编程框架，适合观察 agent orchestration 和 eval loop。 |
...
多 agent 编程框架，适合观察 agent orchestration 和 eval loop。 对用户最有价值的不是单日 star 变化，而是它在 serving、training、agent loop、tool-use 或 Rummy/game-AI 环节中提供的可复用工程抽象。今天 GitHub Search 403，因此日报明确使用 watched repo fallback，避免把 fallback 排名误读为全网真实热度。
