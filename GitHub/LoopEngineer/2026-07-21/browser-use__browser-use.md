...
浏览器自动化 agent，适合 Web 工具调用、端到端 eval 和权限观测。 今日作为 watched repo fallback 被纳入；由于 GitHub Search 403，它不是完整全网排名，但元数据来自 direct repo GET。

## TL;DR

- repo：`browser-use/browser-use`
- stars / forks：105755 / 11642
- language：Python
- updated_at：2026-07-21T00:55:49Z
- stars_delta：181，口径为 direct watched repo fallback vs historical snapshot，非完整全网日增。
...
| 原文链接 | [https://github.com/browser-use/browser-use](https://github.com/browser-use/browser-use) |
...
| 维度 | 判断 | 原因 |
|---|---|---|
| AI Infra 相关性 | 中高 | 浏览器自动化 agent，适合 Web 工具调用、端到端 eval 和权限观测。 |
...
浏览器自动化 agent，适合 Web 工具调用、端到端 eval 和权限观测。 对用户最有价值的不是单日 star 变化，而是它在 serving、training、agent loop、tool-use 或 Rummy/game-AI 环节中提供的可复用工程抽象。今天 GitHub Search 403，因此日报明确使用 watched repo fallback，避免把 fallback 排名误读为全网真实热度。
