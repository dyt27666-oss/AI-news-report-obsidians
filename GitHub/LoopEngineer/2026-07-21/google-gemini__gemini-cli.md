...
Gemini 终端 agent，覆盖 CLI、MCP client/server 与开放工具链。 今日作为 watched repo fallback 被纳入；由于 GitHub Search 403，它不是完整全网排名，但元数据来自 direct repo GET。

## TL;DR

- repo：`google-gemini/gemini-cli`
- stars / forks：106091 / 14291
- language：TypeScript
- updated_at：2026-07-21T00:57:07Z
- stars_delta：17，口径为 direct watched repo fallback vs historical snapshot，非完整全网日增。
...
| 原文链接 | [https://github.com/google-gemini/gemini-cli](https://github.com/google-gemini/gemini-cli) |
...
| 维度 | 判断 | 原因 |
|---|---|---|
| AI Infra 相关性 | 中高 | Gemini 终端 agent，覆盖 CLI、MCP client/server 与开放工具链。 |
...
Gemini 终端 agent，覆盖 CLI、MCP client/server 与开放工具链。 对用户最有价值的不是单日 star 变化，而是它在 serving、training、agent loop、tool-use 或 Rummy/game-AI 环节中提供的可复用工程抽象。今天 GitHub Search 403，因此日报明确使用 watched repo fallback，避免把 fallback 排名误读为全网真实热度。
