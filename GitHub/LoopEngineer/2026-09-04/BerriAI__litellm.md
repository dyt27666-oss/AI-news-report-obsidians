...
## TL;DR
- stars：57972；forks：11150；language：Python。
- updated_at：2026-09-04T00:44:13Z；growth_basis：direct watched repo fallback vs github-stars-2026-09-03.json，非完整全网日增。
...
| language | Python |
| topics | ai-gateway, anthropic, azure-openai, bedrock, gateway, langchain, litellm, llm, llm-gateway, llmops |
...
flowchart TB
  ...
  subgraph System[BerriAI/litellm 核心信号]
    S1[Repo metadata]
    S2[README/Topics]
...
## 专业解读
LLM gateway / proxy / cost tracking / load balancing，适合多模型生产网关和限流治理。 今日由于 GitHub Search 403，本页使用 direct `/repos` 元数据维持知识库连续性；它不是全网增长榜，但适合作为固定 watchlist 的真实元数据快照。
