...
> 一句话结论：`langgenius/dify` 是今日 LoopEngineer 观察项；当前采用 GitHub snapshot/direct fallback 元数据，适合先作为工程选型或机制拆解入口。

## TL;DR
- Stars/Forks：154860 / 24467；语言：TypeScript。
- 更新时间：2026-09-08T01:04:02Z；增长：488（current/direct metadata vs github-stars-2026-09-04.json，非完整全网日增）。
- 重点：Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborative workspace. Deploy on cloud, VPC, or self-hosted, so teams move from prototype to production without rebuilding the stack.
