...
## 一句话结论
图式 agent state machine 框架，适合生产 agent 的恢复、分支和 eval loop。
...
| stars / forks | 37617 / 6308 |
| language | Python |
| updated_at | 2026-07-19T23:21:04Z |
...
## 专业解读
图式 agent state machine 框架，适合生产 agent 的恢复、分支和 eval loop。 对用户而言，核心判断是它能否转化为可执行工程动作：降低推理成本、改善 agent loop 可观测性、形成可复用 benchmark，或给 Point Rummy / RL 环境提供规则和评测抽象。
