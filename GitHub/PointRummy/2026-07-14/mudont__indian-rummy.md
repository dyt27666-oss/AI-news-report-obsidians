# mudont/indian-rummy - 2026-07-14

> 一句话结论：Typescript library for Indian Rummy card game；适合拆解规则状态机、AI opponent、MCTS/RL 或评分器。

## TL;DR

- 来源：GitHub direct `GET /repos` fallback；GitHub Search 今日 403，所以这是 watched repo 信号，不是完整全网排名。
- stars / forks：5 / 0；delta：0（direct GET /repos fallback vs 2026-07-13 snapshot; 非完整全网日增）。
- 更新时间：2025-08-08T21:05:04Z；语言：TypeScript；topics：无。
...
Typescript library for Indian Rummy card game；适合拆解规则状态机、AI opponent、MCTS/RL 或评分器。 由于 GitHub Search 今天从第一批查询开始 403，本条的价值在于“可比 watched baseline”，而不是发现全网新项目。对 AI Infra 工程师来说，稳定 watched repo 能避免 daily radar 被 rate limit 清空；对 Loop Engineer 来说，stars_delta 和 pushed_at 仍能提示哪些 coding-agent 工具值得继续看 release notes、权限模式和上下文策略。
...
```mermaid
mindmap
  root((mudont/indian-rummy))
    Signal
      stars 5
      delta 0
      updated 2025-08-08T21:05:04Z
    Workflow
      Read docs
      Check release
      Try examples
