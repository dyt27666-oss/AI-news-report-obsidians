...
## 一句话结论
IDE agent 执行闭环代表，关注工具调用、MCP、权限和任务恢复。

## TL;DR
- stars：67428；forks：7279；language：TypeScript。
- updated_at：2026-09-04T00:36:31Z；growth_basis：direct watched repo fallback vs github-stars-2026-09-03.json，非完整全网日增。
...
flowchart TB
  ...
  subgraph System[cline/cline 核心信号]
    S1[Repo metadata]
    S2[README/Topics]
...
## 专业解读
IDE agent 执行闭环代表，关注工具调用、MCP、权限和任务恢复。 今日由于 GitHub Search 403，本页使用 direct `/repos` 元数据维持知识库连续性；它不是全网增长榜，但适合作为固定 watchlist 的真实元数据快照。
