# anthropics/claude-code - 2026-07-14

> 一句话结论：Anthropic 终端 agentic coding 工具，适合观察权限模式、代码库理解、git workflow 与多 agent 监控。

## TL;DR

- 来源：GitHub direct `GET /repos` fallback；GitHub Search 今日 403，所以这是 watched repo 信号，不是完整全网排名。
- stars / forks：137733 / 22216；delta：155（direct GET /repos fallback vs 2026-07-13 snapshot; 非完整全网日增）。
- 更新时间：2026-07-14T00:38:01Z；语言：Python；topics：无。
...
| forks | 22216 |
| language | Python |
| updated_at | 2026-07-14T00:38:01Z |
...
Anthropic 终端 agentic coding 工具，适合观察权限模式、代码库理解、git workflow 与多 agent 监控。 由于 GitHub Search 今天从第一批查询开始 403，本条的价值在于“可比 watched baseline”，而不是发现全网新项目。对 AI Infra 工程师来说，稳定 watched repo 能避免 daily radar 被 rate limit 清空；对 Loop Engineer 来说，stars_delta 和 pushed_at 仍能提示哪些 coding-agent 工具值得继续看 release notes、权限模式和上下文策略。
...
```mermaid
mindmap
  root((anthropics/claude-code))
    Signal
      stars 137733
      delta 155
      updated 2026-07-14T00:38:01Z
    Workflow
      Read docs
      Check release
      Try examples
