...
## 一句话结论
OpenHands/OpenHands 今日作为 AIInfra watched repo fallback 进入 Radar；增长数据需按“非完整全网日增”理解。

## TL;DR
- Stars：87786；Forks：11497；Language：TypeScript。
- 最近更新：2026-09-14T01:03:16Z；增长依据：cross-snapshot direct watched repo fallback，非完整全网日增。
...
## 专业解读
OpenHands/OpenHands 的价值不只是 star 数，而是它代表的生态位置：runtime、agent loop、模型框架或业务 baseline。今天的 direct fallback 能保证固定主题不断档，但不能替代完整 GitHub Search。
