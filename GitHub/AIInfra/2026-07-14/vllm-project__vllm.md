# vllm-project/vllm - 2026-07-14

> 一句话结论：LLM serving 高吞吐引擎，今天 direct fallback 仍显示活跃，重点看 KV cache、batching、scheduler 与硬件适配。

## TL;DR

- 来源：GitHub direct `GET /repos` fallback；GitHub Search 今日 403，所以这是 watched repo 信号，不是完整全网排名。
- stars / forks：86163 / 19360；delta：85（direct GET /repos fallback vs 2026-07-13 snapshot; 非完整全网日增）。
- 更新时间：2026-07-14T00:30:03Z；语言：Python；topics：amd, blackwell, cuda, deepseek, deepseek-v3, gpt, gpt-oss, inference, kimi, llama。
...
| forks | 19360 |
| language | Python |
| updated_at | 2026-07-14T00:30:03Z |
| themes | ai_infra_watchlist |
| 原文 | https://github.com/vllm-project/vllm |
...
LLM serving 高吞吐引擎，今天 direct fallback 仍显示活跃，重点看 KV cache、batching、scheduler 与硬件适配。 由于 GitHub Search 今天从第一批查询开始 403，本条的价值在于“可比 watched baseline”，而不是发现全网新项目。对 AI Infra 工程师来说，稳定 watched repo 能避免 daily radar 被 rate limit 清空；对 Loop Engineer 来说，stars_delta 和 pushed_at 仍能提示哪些 coding-agent 工具值得继续看 release notes、权限模式和上下文策略。
...
```mermaid
mindmap
  root((vllm-project/vllm))
    Signal
      stars 86163
      delta 85
      updated 2026-07-14T00:30:03Z
    Workflow
      Read docs
      Check release
      Try examples
