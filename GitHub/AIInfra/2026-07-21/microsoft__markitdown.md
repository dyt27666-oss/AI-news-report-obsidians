...
文档转 Markdown 能直接进入 agent/RAG 上下文，适合构建 coding-agent 文档摄取链路。 今日作为 watched repo fallback 被纳入；由于 GitHub Search 403，它不是完整全网排名，但元数据来自 direct repo GET。

## TL;DR

- repo：`microsoft/markitdown`
- stars / forks：167676 / 12067
- language：Python
- updated_at：2026-07-21T00:58:01Z
- stars_delta：340，口径为 direct watched repo fallback vs historical snapshot，非完整全网日增。
...
| 原文链接 | [https://github.com/microsoft/markitdown](https://github.com/microsoft/markitdown) |
...
| 维度 | 判断 | 原因 |
|---|---|---|
| AI Infra 相关性 | 中高 | 文档转 Markdown 能直接进入 agent/RAG 上下文，适合构建 coding-agent 文档摄取链路。 |
...
文档转 Markdown 能直接进入 agent/RAG 上下文，适合构建 coding-agent 文档摄取链路。 对用户最有价值的不是单日 star 变化，而是它在 serving、training、agent loop、tool-use 或 Rummy/game-AI 环节中提供的可复用工程抽象。今天 GitHub Search 403，因此日报明确使用 watched repo fallback，避免把 fallback 排名误读为全网真实热度。
