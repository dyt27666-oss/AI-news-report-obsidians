...
LLM serving 高吞吐引擎，关注 batching、KV cache、scheduler 与硬件适配。

## TL;DR

- stars / forks：86351 / 19466。
- 语言：Python；最近更新：2026-07-16T00:59:21Z。
...
| forks | 19466 |
| language | Python |
| updated_at | 2026-07-16T00:59:21Z |
...
LLM serving 高吞吐引擎，关注 batching、KV cache、scheduler 与硬件适配。 对用户的价值不在“star 数本身”，而在它暴露的工程接口、调度/执行模式、工具边界和社区采用速度。今天的采集因 GitHub Search 403，采用 watched repo direct fallback；这能保证核心项目不断档，但不能代表完整趋势排名。
