# MCP / Agent Tool-use 标准化继续影响 coding workflow

> 一句话结论：MCP servers、LangGraph、Continue 等项目形成工具调用、agent loop 和私有化 coding assistant 的共同观察面。

## TL;DR
- 来源：MCP / LangGraph / Continue；来源类型：Engineering ecosystem watch。
- 重点：MCP servers、LangGraph、Continue 等项目形成工具调用、agent loop 和私有化 coding assistant 的共同观察面。
- 对我的影响：建议围绕权限、上下文、工具调用和 eval loop 做内部 watchlist。
...
|---|---|
| 大类 | 大厂/工程博客/Research |
| 来源 | MCP / LangGraph / Continue |
| 来源类型 | Engineering ecosystem watch |
...
flowchart TB
  subgraph Source[来源信号]
    S1[MCP / LangGraph / Continue]
    S2[Engineering ecosystem watch]
    S3[公开链接/元数据]
  end
  subgraph Theme[主题映射]
    T1[AI Infra]
...
## 专业解读
MCP servers、LangGraph、Continue 等项目形成工具调用、agent loop 和私有化 coding assistant 的共同观察面。 建议围绕权限、上下文、工具调用和 eval loop 做内部 watchlist。 对 AI Infra 工程师来说，关键不是“热度”，而是它是否改变 runtime、scheduler、工具权限、eval loop、训练/推理成本或业务仿真闭环。
