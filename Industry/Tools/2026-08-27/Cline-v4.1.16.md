...
> 一句话结论：Cline 今日被纳入 Coding 工具矩阵；本次记录聚焦 agent mode、MCP、IDE/CLI、权限和上下文窗口等对 AI coding workflow 的影响。
...
- 原文链接：https://github.com/cline/cline/releases/tag/v4.1.16
- 功能变化： Everything here lands through the SDK bundle, so it applies to windows running that bundle. ### Fixed - Cost estimates are no longer shown for providers billed by a flat-rate subscription (ClinePass, ChatGPT via Codex, and Claude Code). Th
...
| 变化类型 | 今日观察 | 影响 |
|---|---|---|
| Agent 能力 |  Everything here lands through the SDK bundle, so it applies to windows running that bundle. ### Fixed - Cost estimates are no longer shown for providers billed by a flat-rate subscription (ClinePass, ChatGPT via Codex, and Claude Code). Th | 影响能否把复杂任务交给 coding agent 自主闭环 |
