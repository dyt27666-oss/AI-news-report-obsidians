...
## 一句话结论
vLLM、SGLang、TensorRT-LLM、Transformers、PyTorch 仍构成 serving/runtime 观察主线。

## TL;DR
- vLLM、SGLang、TensorRT-LLM、Transformers、PyTorch 仍构成 serving/runtime 观察主线。
- 这些项目直接影响吞吐、延迟、KV cache、scheduler、quantization 和 GPU kernel 选型。
...
## 专业解读
这些项目直接影响吞吐、延迟、KV cache、scheduler、quantization 和 GPU kernel 选型。 对用户而言，关键不是表层热度，而是它能否沉淀成可复用的工程组件：调度、缓存、rollout、tool-use、权限模型、benchmark 或 agent loop 观测。
