# Serving 三件套 watched fallback：vLLM / SGLang / TensorRT-LLM

> 一句话结论：GitHub Search 被限流后，direct repo fallback 仍显示 serving runtime 是今日最稳的 AI Infra 观察主线。

## TL;DR
- 来源：vLLM / SGLang / NVIDIA；来源类型：GitHub / AI Infra watch。
- 重点：GitHub Search 被限流后，direct repo fallback 仍显示 serving runtime 是今日最稳的 AI Infra 观察主线。
...
|---|---|
| 大类 | 大厂/工程博客/Research |
| 来源 | vLLM / SGLang / NVIDIA |
| 来源类型 | GitHub / AI Infra watch |
...
flowchart TB
  subgraph Source[来源信号]
    S1[vLLM / SGLang / NVIDIA]
    S2[GitHub / AI Infra watch]
    S3[公开链接/元数据]
  end
  subgraph Theme[主题映射]
    T1[AI Infra]
...
## 专业解读
GitHub Search 被限流后，direct repo fallback 仍显示 serving runtime 是今日最稳的 AI Infra 观察主线。 建议围绕权限、上下文、工具调用和 eval loop 做内部 watchlist。 对 AI Infra 工程师来说，关键不是“热度”，而是它是否改变 runtime、scheduler、工具权限、eval loop、训练/推理成本或业务仿真闭环。
