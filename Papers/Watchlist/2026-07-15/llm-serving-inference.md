...
> 网页详情：https://github.com/dyt27666-oss/AI-news-report-obsidians/blob/main/Papers/Watchlist/2026-07-15/llm-serving-inference.md
> 返回日报：[[Daily/2026-07-15]]

## 一句话结论

围绕 KV cache、batching、speculative decoding 的论文扫描未完成；今天以 vLLM/SGLang/TensorRT-LLM repo 信号替代工程判断。

## TL;DR

- **它是什么**：arXiv 的今日雷达条目。
- **为什么重要**：对 AI Infra 工程师更可信的当天信号来自 serving repo 活跃度，而不是超时的论文 API。
...
围绕 KV cache、batching、speculative decoding 的论文扫描未完成；今天以 vLLM/SGLang/TensorRT-LLM repo 信号替代工程判断。 对 AI Infra 工程师更可信的当天信号来自 serving repo 活跃度，而不是超时的论文 API。 如果本条目标注为 direct fallback，它只代表固定 watched set 的可信信号，不等于完整 GitHub Search 或全网排名。对工程判断的用法是：把它映射到 serving、agent loop、post-training 或业务仿真中的具体动作，而不是只看 star 数。
