...
> 网页详情：https://github.com/dyt27666-oss/AI-news-report-obsidians/blob/main/Papers/Watchlist/2026-07-13/agentic-rl-post-training.md
> 返回日报：[[Daily/2026-07-13]]

## 一句话结论

arXiv live query 超时，本轮不硬塞弱相关论文；保留 RLHF、GRPO、agent eval 为后续跟踪方向。

## TL;DR

- **它是什么**：arXiv / Semantic Scholar 的今日雷达条目。
- **为什么重要**：避免把物理/泛 ML 噪声放入日报；对 post-training 工作流更重要的是保留失败 provenance 和后续查询入口。
...
arXiv live query 超时，本轮不硬塞弱相关论文；保留 RLHF、GRPO、agent eval 为后续跟踪方向。 避免把物理/泛 ML 噪声放入日报；对 post-training 工作流更重要的是保留失败 provenance 和后续查询入口。 需要注意：本条目来自自动化雷达采集，若标注为 fallback 或间接扫描，则不能当作完整全网排名，只能当作 watched-source 信号。
