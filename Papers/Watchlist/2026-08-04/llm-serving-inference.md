# LLM Serving / inference 论文源扫描 - 2026-08-04
...
| 查询 | `large language model inference` |
...
```mermaid
flowchart TB
  Q[查询: large language model inference] --> S[arXiv / Semantic Scholar]
  S --> F[严格相关性过滤]
...
继续观察 KV cache、batching、scheduler、speculative decoding。 今日因为上游源稳定性不足，日报保留来源链接和主题判断，不写未经核验的论文摘要。
